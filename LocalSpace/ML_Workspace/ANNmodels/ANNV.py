...
def load_csv_files(directory):
    dataset = pd.concat([pd.read_csv(os.path.join(directory, filename)) for filename in os.listdir(directory) if filename.endswith(".csv")],
                        ignore_index=True)
    return dataset
...
def plot_confusion_matrix(y_labels, y_preds, save_location, title):
    print(f"Plotting {title} confusion matrix...")
...
region_paths = [rf"Datasets/FinalDataset/R{i}" for i in range(1, 6)]

# Load CSV files from each region and concatenate them
full_dataset = pd.concat([load_csv_files(region_path) for region_path in region_paths], ignore_index=True)
...
model = Sequential([
    Dense(64, input_dim=X_train_scaled.shape[1], activation='relu', kernel_regularizer=regularizers.l2(0.01)),
    Dropout(0.5),
...
model_checkpoint = ModelCheckpoint(model_save_location + "best_model.h5", save_best_only=True)
